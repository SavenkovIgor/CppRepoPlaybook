...
- **Deterministic tools priority.** If modernization could be done with existing tools like clang-tidy/format, etc, skills will first propose to integrate/enable rule before performing manual edits on the files.
- **Correctness > modernity.** When there is a conflict between making code modern and keeping it correct, correctness takes priority. If some code piece requires rework before it can be modernized, skills will explicitly say so before any
edits.
- **Readability.** Skills must be readable by both a human and an AI: short, focused on one feature or change, written in clear and unambiguous language, grounded in the text of the standard, and industry best practices.
